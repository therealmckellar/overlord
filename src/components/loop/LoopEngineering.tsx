...
const LOOP_TEMPLATES: LoopTemplate[] = [
  { id: 'iterate_refine', name: 'Iterate & Refine', description: 'Generate a draft, then iteratively refine it based on criteria', icon: '🔄', tags: ['writing', 'coding', 'research'], steps: [
    { id: 's1', name: 'Generate Draft', description: 'Create initial output', model: 'poolside/laguna-m.1:free', prompt: 'Generate {{output_type}} based on: {{input}}', outputVar: 'draft' },
    ...
    { id: 's3', name: 'Refine', description: 'Apply improvements', model: 'poolside/laguna-m.1:free', prompt: 'Apply these improvements to the original: {{draft}}. Improvements: {{critique}}', outputVar: 'final' },
  ]},
  { id: 'ab_test', name: 'A/B Test Copy', description: 'Generate two variants, evaluate, pick winner', icon: '🧪', tags: ['sales', 'writing', 'marketing'], steps: [
    { id: 's1', name: 'Variant A', description: 'Generate first variant', model: 'poolside/laguna-m.1:free', prompt: 'Write {{output_type}} variant A: {{input}}', outputVar: 'variant_a' },
    { id: 's2', name: 'Variant B', description: 'Generate second variant with different approach', model: 'nvidia/nemotron-3-ultra-550b-a55b:free', prompt: 'Write {{output_type}} variant B with a completely different approach: {{input}}', outputVar: 'variant_b' },
  ...
  { id: 'scrape_enrich', name: 'Scrape & Enrich', description: 'Extract data from URLs, then enrich with additional research', icon: '🔍', tags: ['sales', 'research', 'enrichment'], steps: [
    ...
    { id: 's2', name: 'Enrich', description: 'Add missing context', model: 'poolside/laguna-m.1:free', prompt: 'Enrich this data with additional context: {{raw_data}}. Find: company size, tech stack, key contacts, recent news.', outputVar: 'enriched', tools: ['web_search'] },
  ...
  { id: 'write_review', name: 'Write & Review', description: 'Write content, review for quality, fix issues', icon: '✍️', tags: ['writing', 'content', 'quality'], steps: [
    { id: 's1', name: 'Write', description: 'Create content', model: 'poolside/laguna-m.1:free', prompt: 'Write {{content_type}}: {{brief}}', outputVar: 'draft' },
    ...
    { id: 's3', name: 'Fix', description: 'Apply fixes', model: 'poolside/laguna-m.1:free', prompt: 'Apply these fixes to the draft:\nDraft: {{draft}}\nFixes: {{review}}', outputVar: 'final' },
  ]},
  { id: 'deep_research', name: 'Deep Research', description: 'Multi-source research with synthesis', icon: '🔬', tags: ['research', 'analysis'], steps: [
    ...
    { id: 's2', name: 'Deep Dive', description: 'Go deeper on key areas', model: 'poolside/laguna-m.1:free', prompt: 'Based on these initial findings, identify gaps and research deeper: {{initial_findings}}', outputVar: 'deep_findings', tools: ['web_search'] },
  ...
  { id: 'code_build_test', name: 'Build → Test → Fix', description: 'Build code, test it, fix failures', icon: '💻', tags: ['coding', 'development'], steps: [
    { id: 's1', name: 'Build', description: 'Implement the feature', model: 'poolside/laguna-m.1:free', prompt: 'Implement: {{spec}}', outputVar: 'code', tools: ['terminal', 'file_read', 'file_write'] },
    ...
    { id: 's3', name: 'Fix', description: 'Fix test failures', model: 'poolside/laguna-m.1:free', prompt: 'Fix these test failures:\nCode: {{code}}\nTest results: {{test_results}}', outputVar: 'fixed_code', tools: ['terminal', 'file_read', 'file_write'] },
  ]},
  { id: 'sdr_outreach', name: 'SDR Outreach Loop', description: 'Research prospect, write outreach, optimize', icon: '💰', tags: ['sales', 'sdr', 'outreach'], steps: [
    ...
    { id: 's2', name: 'Write Outreach', description: 'Craft personalized message', model: 'poolside/laguna-m.1:free', prompt: 'Write a personalized cold email based on this intel: {{prospect_intel}}\nProduct: {{product}}\nTone: professional but conversational', outputVar: 'email_draft' },
  ...
  { id: 'transform_chain', name: 'Transform Chain', description: 'Sequential data transformation pipeline', icon: '⛓️', tags: ['operations', 'data', 'pipeline'], steps: [
    ...
    { id: 's2', name: 'Transform', description: 'Transform to target format', model: 'poolside/laguna-m.1:free', prompt: 'Transform this data into {{target_format}}: {{extracted}}', outputVar: 'transformed' },
